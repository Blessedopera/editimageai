...
// SIMPLIFIED LOGIN - Just try to login, if email not confirmed error, we'll handle it differently
app.post('/auth/login', async (req, res) => {
  try {
    ...
    // If email not confirmed, we need to confirm it via admin
    if (authError && authError.message.includes('Email not confirmed')) {
      console.log('Email not confirmed, attempting to confirm via admin...');
      ...
      try {
        ...
        if (retryAuthError) {
          console.error('Retry login error:', retryAuthError);
          return res.status(401).json({ error: 'Invalid email or password' });
        }

        // Use the retry auth data
        const finalAuthData = retryAuthData;

        console.log('Auth successful after confirmation, fetching profile for:', finalAuthData.user.id);
        ...
        // Create JWT token
        const token = jwt.sign(
          { userId: finalAuthData.user.id, email: finalAuthData.user.email },
          process.env.JWT_SECRET,
          { expiresIn: '24h' }
        );
        ...
        console.error('Email confirmation process error:', confirmError);
        return res.status(500).json({ error: 'Failed to confirm email' });
      }
    }
...
app.listen(port, () => {
  ...
  console.log('- Email Confirmation Auto-Fix');
});
